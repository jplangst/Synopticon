...
void FLiveLinkNatNetSource::SetSourceSettings( UOptitrackLiveLinkSourceSettings& Settings )
{
	...
	SetTimecodeProviderSubject( Settings.SubjectName );
	...
	CameraSubjects = Settings.CameraSubjects;
	...
	for( int iDesc = 0; iDesc < NNDataDescs->nDataDescriptions; ++iDesc )
	{
		...
		if( dataDesc.type == Descriptor_Asset )
		{
			...
			if( assetDesc.AssetType == AssetType_TrainedMarkerset )
			{
				...
				const FName assetName = FName( "TMarkerset_" + assetNameString);
		...
		if( dataDesc.type == Descriptor_Skeleton )
		{
			...
			// Figure out the Bone Structure
			TArray<FName> boneNames;
			boneNames.Reserve(skelDesc.nRigidBodies + 2);

			TArray<int32> boneParents;
			boneParents.Reserve(skelDesc.nRigidBodies + 2);

			boneNames.Emplace( "Origin" );
			boneParents.Emplace( -1 );
			...
			for( int i = 0; i < skelDesc.nRigidBodies; ++i )
			{
				...
				if( boneDesc.parentID > 0 )
				{
					for( int p = 0; p < skelDesc.nRigidBodies; p++ )
					{
						...
						{
							// +1 for having Root bone and origin
							parentIndex = p + 2;
							break;
						}
					}
		...
		if( dataDesc.type == Descriptor_RigidBody )
		{
			...
			// For transform role
			if( CameraSubjects.Contains( { LLSourceGuid, rbName } ) && CameraSubjects[ { LLSourceGuid, rbName } ] )
			{
				FLiveLinkStaticDataStruct rigidBodyStaticData = FLiveLinkStaticDataStruct( FLiveLinkCameraStaticData::StaticStruct() );
				LLClient->PushSubjectStaticData_AnyThread( { LLSourceGuid, rbName }, ULiveLinkCameraRole::StaticClass(), MoveTemp( rigidBodyStaticData ) );
			}
			else
			{
...
//	const FVector boneScale = FVector( 1.0, 1.0, 1.0 );
	...
	for( int i = 0; i < NewFrame->nAssets; ++i )
	{
		...
		if( TMarkersetName != nullptr )
		{
			...
			transforms[ 0 ] = FTransform(); // this is the root

			for( int rbIndex = 0; rbIndex < assetData.nRigidBodies; ++rbIndex )
			{
				...
				transforms[ rbIndex + 1 ] = FTransform( rot, pos );
			}
	...
	for ( int iSkel = 0; iSkel < NewFrame->nSkeletons; ++iSkel )
	{
		...
		FLiveLinkAnimationFrameData& animFrame = *frameDataStruct.Cast<FLiveLinkAnimationFrameData>();
		TArray<FTransform>& transforms = animFrame.Transforms;
		transforms.SetNumZeroed( skelData.nRigidBodies + 2 );
		...
		for ( int rbIndex = 1; rbIndex < skelData.nRigidBodies; ++rbIndex )
		{
			...
			transforms[rbIndex + 2] = FTransform( rot, pos );
		}
	...
	for( int rbIndex = 0; rbIndex < NewFrame->nRigidBodies; rbIndex++ )
	{
		...
		bool isCamera = LLClient->GetSubjectRole_AnyThread( { LLSourceGuid, rbName } ) == ULiveLinkCameraRole::StaticClass();
		UScriptStruct* structType;
		if( isCamera )
		{
			structType = FLiveLinkCameraFrameData::StaticStruct();
		}
		else
		{
			structType = FLiveLinkTransformFrameData::StaticStruct();
		}
		FLiveLinkFrameDataStruct transformFrameDataStruct( structType );
		FLiveLinkTransformFrameData& transformFrame = isCamera ?
			*transformFrameDataStruct.Cast<FLiveLinkCameraFrameData>() : *transformFrameDataStruct.Cast<FLiveLinkTransformFrameData>();
