...
{
	FString assetName = key.SubjectName.ToString();
	if( assetName.StartsWith( "TMarkerset_" ) )
	{
		assetName.RemoveFromStart( "TMarkerset_" );
	}
	mAssetComponents.Add( key, CreateDefaultSubobject<UOptitrackLiveLinkComponent>( FName( assetName ) ) );
	...
	mNameDict.FindOrAdd( assetName ) = key;
	if( mMarkerDict.Contains( assetName ) )
	{
		mAssetComponents[ key ]->mMarkerSubjectKey = mMarkerDict[ assetName ];
	}
}
...
void AOptitrackLiveLinkDisplay::AddComponentRealtime( const FLiveLinkSubjectKey& key )
{
	...
	mAssetComponents.Add( key, NewObject<UOptitrackLiveLinkComponent>( this, FName( assetName ) ) );
	mAssetComponents[ key ]->SetupAttachment( RootComponent );
	this->AddInstanceComponent( mAssetComponents[ key ] );
	this->AddOwnedComponent( mAssetComponents[ key ] );
	mAssetComponents[ key ]->RegisterComponent();
	mAssetComponents[ key ]->mParentActor = this;
	...
	// This last section is for reapplying all the actor properties to the new components
	mAssetComponents[ key ]->mMarkerSize = mLabeledMarkerSize;
	mAssetComponents[ key ]->mMarkerColor = mLabeledMarkerColor;
	mAssetComponents[ key ]->mDisplayMarkers = mDrawLabeledMarkers;
	
	if( mTMarkersetData.Contains( key ) )
	{
		mAssetComponents[ key ]->mLineThicknessScale = mTMarkersetLineThickness;
		mAssetComponents[ key ]->mAssetColor = mTMarkersetColor;
		mAssetComponents[ key ]->mDisplayAsset = mDrawTMarkersets;
	}
	else if( mSkeletonData.Contains( key ) )
	{
		mAssetComponents[ key ]->mLineThicknessScale = mSkeletonLineThickness;
		mAssetComponents[ key ]->mAssetColor = mSkeletonColor;
		mAssetComponents[ key ]->mDisplayAsset = mDrawSkeletons;
	}
	else if( mRigidBodyData.Contains( key ) )
	{
		mAssetComponents[ key ]->mLineThicknessScale = mRigidBodyLineThickness;
		mAssetComponents[ key ]->mAssetColor = mRigidBodyColor;
		mAssetComponents[ key ]->mDisplayAsset = mDrawRigidBodies;
	}
	else
	{
		mAssetComponents[ key ]->mLineThicknessScale = mLineThicknessScale;
		mAssetComponents[ key ]->mAssetColor = mAssetColor;
		mAssetComponents[ key ]->mDisplayAsset = mDrawAssets;
	}
}
...
bool AOptitrackLiveLinkDisplay::RegisterForLiveLinkData()
{
	...
	if( ModularFeatures.IsModularFeatureAvailable( ILiveLinkClient::ModularFeatureName ) )
	{
		...
		// Delete any components that aren't in Live Link
		for( const auto& it : mAssetComponents )
		{
			if( !subjectKeys.Contains( it.Key ) )
			{
				it.Value->DetachFromComponent( FDetachmentTransformRules::KeepRelativeTransform );
				this->RemoveInstanceComponent( it.Value );
				this->RemoveOwnedComponent( it.Value );
				mAssetComponents.Remove( it.Key );
			}
		}
	}
	...
	const TMap<FLiveLinkSubjectKey, TObjectPtr<UOptitrackLiveLinkComponent>>& components ) const
{
	for( const auto& it : skeletons )
	{
		...
		if( parentBones.Contains( it.Key ) )
		{
			...
			// Draw bone segments
			for( int32 i = 2; i < boneParents.Num(); i++ ) // start at 2 to avoid the hip->root line?
			{
				int32 parentIndex = boneParents[i];
				if( i < transforms.Num() && parentIndex >= 0 && parentIndex < transforms.Num() )
				{
