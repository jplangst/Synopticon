...
void Levmar::levmarq_init(LMstat *lmstat)
{
	lmstat->verbose = 0;
	lmstat->max_it = 20000;
...
void Levmar::initArray(int row, int col, double** arr)
{
	for (int i = 0; i < row; i++)
		arr[i] = new double[col];
}
void Levmar::deleteArray(int row, int col, double** arr)
{
	for (int i = 0; i < row; i++)
		delete[] arr[i];
	delete[] arr;
...
*/
int Levmar::levmarq(int npar, double *par, int ny, double *y, double *dysq,
	...
	void *fdata, LMstat *lmstat)
{
	...
	double **h, **ch;
	double *g, *d, *delta, *newpar;
	h = new double*[npar];
	initArray(npar, npar, h);
	ch = new double*[npar];
	initArray(npar, npar, ch);
	g = new double[npar];
	d = new double[npar];
	...
	deleteArray(npar, npar, h);
	deleteArray(npar, npar, ch);
	delete [] g;
	delete[] d;
	delete[] delta;
	delete[] newpar;
...
double Levmar::error_func(double *par, int ny, double *y, double *dysq,
...
*/
void Levmar::solve_axb_cholesky(int n, double** l, double* x, double* b)
{
	int i, j;
	double sum;
...
*/
int Levmar::cholesky_decomp(int n, double** l, double** a)
{
	int i, j, k;
	double sum;
