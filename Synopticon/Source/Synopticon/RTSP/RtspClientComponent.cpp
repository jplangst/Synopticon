...
bool URtspClientComponent::describeGet() {
    if (!IsConnected) {
    ...
    auto response = send_request("GET", path_, {}, ec);
    if (ec) {
        return false;
    }
...
bool URtspClientComponent::setup(int rtp_port, int rtcp_port) {
    ...
    //auto response = send_request("SETUP", path_, extra_headers, ec); //Was this
    //FString setupPath = "/test/trackID=0";
    //auto response = send_request("SETUP", TCHAR_TO_UTF8(*setupPath), extra_headers, ec);
    if (ec) {
...
bool URtspClientComponent::connect_thread_func() {
    ...
    AsyncTask(ENamedThreads::GameThread, [this]() {
        ...
        // TODO testing if this is an ok place to call describe setup etc?
        describeGet();
        describe();
        setup(video_port_, 8556);
        });
